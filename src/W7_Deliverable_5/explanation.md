...
- `data/interim/airbnb_with_area_codes.csv`: Cleaned Airbnb Christchurch listings enriched with spatial area codes.
- `data/interim/bond_data_clean.csv`: Cleaned Tenancy Services quarterly rental bond dataset.

- **Spatial Boundaries:** Stats NZ Statistical Area 2 2019 Generalised Layer (Layer ID `98970`).

The final integrated dataset is saved to `data/processed/final_airbnb_bond_merged.csv`. The integration code is in `merge_datasets.py` (and `merge_datasets_sql.py`, which repeats the join in SQLite as a cross-check).
