...
def get_quarter_start(date):
    """Rounds a given month down to the start of its respective quarter."""
    if pd.isna(date): 
        return None
    # Math to find the start month of the quarter (e.g., Nov (11) -> Oct (10))
...
def merge_datasets():
    ...
    # Align timeframe keys: Convert Airbnb monthly dates to Quarterly starts
    ...
    # PREVENT DUPLICATION: Isolate the total median rent for the area
    bond_totals_df = bond_df[(bond_df['Dwelling Type'] == 'ALL') & (bond_df['Number Of Beds'] == 'ALL')].copy()

    print("Executing Left Join...")
    # Perform Left Join to preserve all 28,795 Airbnb observations
    merged_df = pd.merge(
        airbnb_df,
        bond_totals_df,
...
if __name__ == "__main__":
    ...
    # --- VERIFICATION STEP (Pointing to the correct 'data/' folder) ---
